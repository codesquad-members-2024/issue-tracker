...
function MilestoneArea({ handleFetch, handleClearTimeOut, milestoneId }: ProsType) {
	const [open, setOpen] = useState(false);
	const previousItem = useRef<HTMLInputElement | null>(null);
	const currentMilestone = useRef<Milestone | null>(null);
	...
	const handleCheckedItems = ({ target }: React.ChangeEvent<HTMLInputElement>, idx: number) => {
		setOpen(false);
		currentMilestone.current = idx ? milestones[idx - 1] : null;
		if (previousItem.current) previousItem.current.checked = false;

		previousItem.current = target;

		if (!idx) {
			milestoneId.current = null;
			return;
		}
		milestoneId.current = milestones[idx - 1].id;
	};
	...
	return (
		...
		>
			...
			<div
				className={`flex flex-wrap transition-[padding] w-[80%] ${milestoneId ? "pb-3" : "pb-0"}`}
			>
				{currentMilestone.current ? (
					<ProgressWithLabel
						percent={currentMilestone.current.milestoneProgress}
						name={currentMilestone.current.name}
					></ProgressWithLabel>
				) : (
					<></>
				)}
