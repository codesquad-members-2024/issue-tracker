...
function AssigneeArea({ handleFetch, handleClearTimeOut, assignees, issueId }: ProsType) {
	...
	useEffect(() => {
		...
		const assigneeIds = currentAssignees.map(({ memberId }) => memberId);
		mutate({
			assigneeIds,
		});
	}, [currentAssignees, mutate]);

	const handleShowDropdownPanel = () => {
		setOpen(false);
		setCurrentAssignees(Object.values(checkedItems.current).map((idx) => members[idx]));
	};
